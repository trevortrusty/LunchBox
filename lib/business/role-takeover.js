 ...
 */
export async function executeRoleSwap(prisma, { restPeriodId, reliefAssociateId, noRelief }) {
  ...
  if (restPeriod.status === 'OUT') throw new RoleSwapError('Associate is already out')
  if (restPeriod.status === 'COMPLETED') throw new RoleSwapError('Rest period already completed')
  ...
  if (!reliefShift) throw new RoleSwapError('Relief associate does not have an active shift')
  ...
  if (reliefActiveRest) throw new RoleSwapError('Relief associate is currently on break')

  // Check relief associate is not already covering another shift
  const reliefShiftFull = await prisma.shift.findUnique({ where: { id: reliefShift.id } })
  if (reliefShiftFull.temporarilyCoveringShiftId) {
    throw new RoleSwapError('Relief associate is already covering another shift')
  }

  // No relief chains: departing shift should not itself be covering someone
  if (departingShift.temporarilyCoveringShiftId) {
    throw new RoleSwapError('Cannot create relief chains: departing associate is already covering another shift')
  }
...
export async function executeReturn(prisma, { restPeriodId }) {
  ...
  if (!restPeriod) throw new RoleSwapError('Rest period not found')
  if (restPeriod.status !== 'OUT') throw new RoleSwapError('Associate is not currently out')
