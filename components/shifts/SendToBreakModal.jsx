...
export default function SendToBreakModal({
  isOpen,
  onClose,
  shift,
  restPeriod,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (noRelief) body.noRelief = true;
        ...
        body.reliefAssociateId = reliefAssociates.find(
          (s) => s.id === selectedRelief,
        )?.associateId;
      ...
      toast.success(
        `${shift.associate?.name} sent to ${restPeriod.type.toLowerCase()}`,
      );
      onComplete();
    } catch {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Send to ${restLabel} — ${shift.associate?.name}`}
    >
