...
export default function ReturnFromBreakModal({ isOpen, onClose, shift, restPeriod, onComplete }) {
  ...
  const handleReturn = async () => {
    ...
    try {
      ...
      toast.success(`${shift.associate?.name} returned from ${restPeriod.type.toLowerCase()}`)
      onComplete()
    } catch {
    ...
    >
      <div className="space-y-4">
        <p className="text-sm text-gray-600">
          Confirm that <strong>{shift.associate?.name}</strong> has returned from their{' '}
          {restLabel}.
          {restPeriod?.relievedByAssociate && (
            <span>
              {' '}
              Relief associate <strong>{restPeriod.relievedByAssociate.name}</strong> will be
