...
function getRestButtonStyle(status) {
  switch (status) {
    case 'DUE': return 'bg-amber-100 text-amber-800 border-amber-300 hover:bg-amber-200'
    case 'OUT': return 'bg-blue-100 text-blue-800 border-blue-300 hover:bg-blue-200'
    case 'COMPLETED': return 'bg-green-100 text-green-800 border-green-300 cursor-default'
...
function getRestButtonLabel(rest) {
  ...
  switch (rest.status) {
    case 'DUE': return `${typeLabel} DUE`
    case 'OUT': return `${typeLabel} OUT`
    case 'COMPLETED': return `${typeLabel} Done`
    default: return `${typeLabel} ${time}`
  }
}
...
function getRowStyle(shift) {
  ...
  const allDone = rests.length > 0 && rests.every((r) => r.status === 'COMPLETED')
...
export default function ShiftRow({ shift, onRefresh }) {
  ...
  const handleRestClick = (rest) => {
    if (rest.status === 'COMPLETED') return
    if (rest.status === 'OUT') {
    ...
    <>
      <tr className={`${getRowStyle(shift)} transition-colors`}>
        <td className="px-4 py-3">
          <div className="font-medium text-gray-900">{shift.associate?.name}</div>
          {shift.department && (
            <div className="text-xs text-gray-400">{shift.department.name}</div>
          )}
        </td>
        <td className="px-4 py-3 text-gray-600">
          {shift.currentRole || <span className="text-gray-300 italic">No role</span>}
      ...
      {sendBreakRest && (
        <SendToBreakModal
          ...
          onComplete={() => { setSendBreakRest(null); onRefresh() }}
        />
      )}
      ...
      {returnBreakRest && (
        <ReturnFromBreakModal
          ...
          onComplete={() => { setReturnBreakRest(null); onRefresh() }}
        />
      )}
      ...
      {showChangeRole && (
        <ChangeRoleModal
          ...
          shift={shift}
          onComplete={() => { setShowChangeRole(false); onRefresh() }}
        />
      )}
      ...
      {showAssignTask && (
        <AssignTaskModal
          ...
          shift={shift}
          onComplete={() => { setShowAssignTask(false); onRefresh() }}
        />
      )}
    </>
  )
